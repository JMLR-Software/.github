...
## 2. Add a root `NOTES.md`

Every repo carries a root `NOTES.md` with three headings: `## Links`, `## Open items`, and
`## Done`. Open items are `- [ ]` checkboxes; a deadline is written inline as
`(due: 2026-11-03)`.

This is what HQ reads. A repo without it is invisible on the dashboard at `hq.jmlr.dev`,
which means its outstanding work is invisible too. See `~/Coding/hq/README.md`.
...
## 4. Protection is enforced at the org level, not by this checklist (nothing to do)
...
## 5. Hosting is Workers, never Pages (required, every site repo)
